	...
	<li>
		...
		<div class="userInformation">
			...
			<div v-if="isLogin && isMyacount(user.id)">
				<button @click="follow(user.id)" v-show="!followees.includes(user.id)">フォローする</button>
				<button @click="unfollow(user.id)" v-show="followees.includes(user.id)" class="unfollow">
					フォロー解除
				</button>
			</div>
		</div>
