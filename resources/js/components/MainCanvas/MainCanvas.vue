<template>
  <div id="MainCanvas">
    <ul>
      <Dot v-for="item in allCanvasDot" v-bind:key="item" v-bind:dotid="item" v-bind:drawingJudgement="drawingJudgement" v-bind:lineDotVolume="lineDotVolume" v-bind:color="color[item-1]" @mousedown.native="dragStart" @mouseup.native="dragEnd" @saveProduct="saveProduct"></Dot>
    </ul>
  </div>
</template>
...
export default{
  ...
  methods:{
    ...
    async saveConnect(){
      ...
      const response = await axios.post('/api/products/save',{currentProduct:this.currentProduct,dots:this.dots})
      this.dots.length = 0
    }
  }
}
